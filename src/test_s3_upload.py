...
def test_s3_operations():
    ...
    print(
        f"Uploading file object to {fileobj_key} with custom metadata and content type..."
    )
    ...
    result = s3_storage.upload_fileobj(
        file_obj, fileobj_key, metadata=metadata, content_type="text/plain"
    )

    if result:
        print(
            f"✅ File object upload successful with metadata! File uploaded to: {s3_storage.bucket_name}/{fileobj_key}"
        )
        passed_tests += 1
    else:
    ...
    result = s3_storage.download_file(
        file_key, str(download_path), callback=progress_callback
    )
