        ...
        def wrapper(*args, **kwargs):
            ...
            while True:
                try:
                    ...
                    logger.warning(f"Retrying {func.__name__} after error: {e}. Retry {retries}/{max_retries} in {delay:.2f}s")
                    time.sleep(delay)
...
class S3DocumentStorage:
    ...
    def __init__(self, bucket_name: Optional[str] = None, aws_access_key_id: Optional[str] = None, 
                 aws_secret_access_key: Optional[str] = None, endpoint_url: Optional[str] = None,
                 region_name: Optional[str] = None, max_pool_connections: int = 10,
                 connect_timeout: int = 5, read_timeout: int = 60, retries: int = 3):
        """
        ...
        if not self.bucket_name:
            raise ValueError("S3 bucket name must be provided either as a parameter or as S3_BUCKET_NAME environment variable")
        ...
        logger.info(f"Initialized S3DocumentStorage for bucket '{self.bucket_name}' at {self.endpoint_url}")
    ...
    def _get_multipart_config(self, file_size: Optional[int] = None) -> Dict[str, Any]:
        ...
        if file_size and file_size > MULTIPART_THRESHOLD:
            ...
            chunk_size = max(min(calculated_chunk_size, 100 * 1024 * 1024), MULTIPART_CHUNKSIZE)
    ...
    @retry_s3_operation(max_retries=3, error_codes=['RequestTimeout', 'ConnectionError', 'InternalError'])
    def upload_file(self, file_path: str, s3_key: str, metadata: Optional[Dict[str, str]] = None,
        ...
        try:
            ...
            logger.info(f"Uploading {file_path} ({file_size} bytes) to {self.bucket_name}/{s3_key}")
    ...
    def upload_fileobj(self, file_obj: BinaryIO, s3_key: str, metadata: Optional[Dict[str, str]] = None,
        ...
        try:
            ...
            logger.error(f"Error uploading file object to {self.bucket_name}/{s3_key}: {e}")
            return False

    @retry_s3_operation(max_retries=3)
    def download_file(self, s3_key: str, local_path: str, callback: Optional[Callable[[int], None]] = None) -> bool:
        """
        ...
        try:
            ...
            if not self.file_exists(s3_key):
                logger.error(f"File {s3_key} does not exist in bucket {self.bucket_name}")
                return False
